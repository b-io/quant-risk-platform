...
void SQLiteStorageBackend::store_factor_definition(const domain::FactorDefinition& factor) {
    const char* sql = "INSERT OR REPLACE INTO factor_definitions (factor_id, factor_type, "
                      "shock_measure, currency, curve_id, tenor, quote_ids_json, description) "
                      "VALUES (?, ?, ?, ?, ?, ?, ?, ?);";
    sqlite3_stmt* stmt;
...
void SQLiteStorageBackend::store_factor_observation(const domain::FactorObservation& obs) {
    const char* sql = "INSERT OR REPLACE INTO factor_observations (factor_id, market_date, level, "
                      "move, move_unit) VALUES (?, ?, ?, ?, ?);";
    sqlite3_stmt* stmt;
    if (sqlite3_prepare_v2(db_, sql, -1, &stmt, nullptr) != SQLITE_OK) {
        throw std::runtime_error(fmt::format("Failed to prepare statement: {}", sqlite3_errmsg(db_)));
    }
...
SQLiteStorageBackend::load_factor_definitions(const std::string& /*portfolio_id*/) {
    std::vector<domain::FactorDefinition> factors;
    const char* sql = "SELECT factor_id, factor_type, shock_measure, currency, curve_id, tenor, "
                      "quote_ids_json, description FROM factor_definitions;";
    sqlite3_stmt* stmt;
    if (sqlite3_prepare_v2(db_, sql, -1, &stmt, nullptr) != SQLITE_OK) {
        throw std::runtime_error(fmt::format("Failed to prepare statement: {}", sqlite3_errmsg(db_)));
    }
    ...
    while (sqlite3_step(stmt) == SQLITE_ROW) {
        ...
        if (q_json)
            f.quote_ids = nlohmann::json::parse(q_json).get<std::vector<std::string>>();
...
std::vector<domain::FactorObservation>
SQLiteStorageBackend::load_factor_history(const std::vector<std::string>& factor_ids,
    ...
    std::string sql = fmt::format("SELECT factor_id, market_date, level, move, move_unit FROM factor_observations "
                                  "WHERE factor_id IN {} AND market_date BETWEEN ? AND ? ORDER BY market_date ASC;",
                                  in_clause);
...
void SQLiteStorageBackend::store_factor_binding(const domain::FactorBinding& binding) {
    const char* sql = "INSERT OR REPLACE INTO factor_quote_bindings (factor_id, quote_id, "
                      "shock_measure, weight, transform, selector_json) "
                      "VALUES (?, ?, ?, ?, ?, ?);";
    sqlite3_stmt* stmt;
    if (sqlite3_prepare_v2(db_, sql, -1, &stmt, nullptr) != SQLITE_OK) {
        throw std::runtime_error(fmt::format("Failed to prepare statement: {}", sqlite3_errmsg(db_)));
...
SQLiteStorageBackend::load_factor_bindings(const std::vector<std::string>& factor_ids) {
    ...
    std::string sql = fmt::format("SELECT factor_id, quote_id, shock_measure, weight, transform, "
                                  "selector_json FROM factor_quote_bindings "
                                  "WHERE factor_id IN {};",
                                  in_clause);
