...
public:
    EquityForwardInstrument(double quantity,
        ...
        : quantity_(quantity), contract_forward_(contract_forward), direction_sign_(direction_sign),
          maturity_date_(std::move(maturity_date)), spot_quote_(std::move(spot_quote)),
          dividend_yield_quote_(std::move(dividend_yield_quote)), borrow_rate_quote_(std::move(borrow_rate_quote)),
...
protected:
    void performCalculations() const override {
        ...
        const double rate = time_to_maturity > 0.0 && discount > 0.0 ? -std::log(discount) / time_to_maturity : 0.0;
...
public:
    EquityFutureInstrument(double quantity,
        ...
        : quantity_(quantity), contract_size_(contract_size), reference_price_(reference_price),
          ...
          future_quote_(std::move(future_quote)), spot_quote_(std::move(spot_quote)),
          dividend_yield_quote_(std::move(dividend_yield_quote)), borrow_rate_quote_(std::move(borrow_rate_quote)),
          discount_curve_(std::move(discount_curve)) {
        if (future_quote_)
...
public:
    EquityOptionInstrument(double quantity,
                           double strike_price,
                           double direction_sign,
                           bool is_put,
                           bool american,
                           ...
                           QuantLib::ext::shared_ptr<QuantLib::YieldTermStructure> discount_curve)
        : quantity_(quantity), strike_price_(strike_price), direction_sign_(direction_sign), is_put_(is_put),
          american_(american), expiry_date_(std::move(expiry_date)), settlement_date_(std::move(settlement_date)),
          spot_quote_(std::move(spot_quote)), dividend_yield_quote_(std::move(dividend_yield_quote)),
          borrow_rate_quote_(std::move(borrow_rate_quote)), volatility_quote_(std::move(volatility_quote)),
          discount_curve_(std::move(discount_curve)) {
        registerWith(spot_quote_);
        registerWith(dividend_yield_quote_);
        registerWith(borrow_rate_quote_);
...
protected:
    void performCalculations() const override {
        ...
        const double rate = time_to_expiry > 0.0 && discount > 0.0 ? -std::log(discount) / time_to_expiry : 0.0;
...
private:
    double intrinsic(double spot) const {
        return is_put_ ? std::max(strike_price_ - spot, 0.0) : std::max(spot - strike_price_, 0.0);
    }

    double
    european_value(double spot, double rate, double effective_yield, double volatility, double time_to_expiry) const {
        ...
        const double d1 = (std::log(spot / strike_price_) +
                           (rate - effective_yield + 0.5 * volatility * volatility) * time_to_expiry) /
                          std_dev;
        ...
        return is_put_ ? strike_price_ * df * normal_cdf(-d2) - spot * yield_df * normal_cdf(-d1)
...
EquityInstrumentFactory::create_equity_spot(const domain::EquitySpotTrade& trade,
...
QuantLib::ext::shared_ptr<QuantLib::Instrument>
EquityInstrumentFactory::create_equity_forward(const domain::EquityForwardTrade& trade,
                                               const analytics::PricingContext& context) {
    if (trade.maturity_date.empty()) {
        return nullptr;
    }

    auto spot_quote = find_quote_handle(context,
                                        trade.spot_quote_id,
                                        trade.underlier,
                                        "SPOT",
    ...
    return QuantLib::ext::make_shared<EquityForwardInstrument>(trade.quantity,
                                                               trade.forward_price,
...
QuantLib::ext::shared_ptr<QuantLib::Instrument>
EquityInstrumentFactory::create_equity_future(const domain::EquityFutureTrade& trade,
                                              const analytics::PricingContext& context) {
    if (trade.maturity_date.empty()) {
        return nullptr;
    }
    ...
    return QuantLib::ext::make_shared<EquityFutureInstrument>(trade.quantity,
                                                              trade.contract_size,
...
QuantLib::ext::shared_ptr<QuantLib::Instrument>
EquityInstrumentFactory::create_equity_option(const domain::EquityOptionTrade& trade,
                                              const analytics::PricingContext& context) {
    ...
    return QuantLib::ext::make_shared<EquityOptionInstrument>(trade.quantity,
                                                              trade.strike_price,
                                                              long_short_direction_sign(trade.direction),
