...
public:
    FxSpotInstrument(double notional,
                     double reference_rate,
                     double direction_sign,
                     QuantLib::ext::shared_ptr<QuantLib::SimpleQuote> spot_quote)
        : notional_(notional), reference_rate_(reference_rate), direction_sign_(direction_sign),
...
public:
    FxForwardInstrument(double notional,
        ...
        : notional_(notional), contract_forward_(contract_forward), direction_sign_(direction_sign),
          maturity_date_(std::move(maturity_date)), spot_quote_(std::move(spot_quote)),
          forward_quote_(std::move(forward_quote)), forward_points_quote_(std::move(forward_points_quote)),
          domestic_curve_(std::move(domestic_curve)), foreign_curve_(std::move(foreign_curve)) {
        registerWith(spot_quote_);
        if (forward_quote_)
            registerWith(forward_quote_);
        if (forward_points_quote_)
            registerWith(forward_points_quote_);
        if (domestic_curve_)
            registerWith(domestic_curve_);
        if (foreign_curve_)
            registerWith(foreign_curve_);
    }
...
protected:
    void performCalculations() const override {
        const double market_forward = forward_rate_from_quotes(spot_quote_,
                                                               forward_quote_,
...
public:
    FxSwapInstrument(double notional,
                     double near_rate,
                     double far_rate,
                     double direction_sign,
                     ...
                     QuantLib::ext::shared_ptr<QuantLib::YieldTermStructure> domestic_curve,
                     QuantLib::ext::shared_ptr<QuantLib::YieldTermStructure> foreign_curve)
        : notional_(notional), near_rate_(near_rate), far_rate_(far_rate), direction_sign_(direction_sign),
          near_date_(std::move(near_date)), far_date_(std::move(far_date)), spot_quote_(std::move(spot_quote)),
          ...
          far_forward_points_quote_(std::move(far_forward_points_quote)), domestic_curve_(std::move(domestic_curve)),
          foreign_curve_(std::move(foreign_curve)) {
        registerWith(spot_quote_);
        if (near_forward_quote_)
            registerWith(near_forward_quote_);
        if (near_forward_points_quote_)
            registerWith(near_forward_points_quote_);
        if (far_forward_quote_)
            registerWith(far_forward_quote_);
        if (far_forward_points_quote_)
            registerWith(far_forward_points_quote_);
        if (domestic_curve_)
            registerWith(domestic_curve_);
        if (foreign_curve_)
            registerWith(foreign_curve_);
...
protected:
    void performCalculations() const override {
        const double near_forward = forward_rate_from_quotes(spot_quote_,
                                                             near_forward_quote_,
                                                             near_forward_points_quote_,
                                                             domestic_curve_,
                                                             foreign_curve_,
                                                             near_date_);
        const double far_forward = forward_rate_from_quotes(spot_quote_,
                                                            far_forward_quote_,
...
public:
    FxOptionInstrument(double notional,
                       double strike_rate,
                       double direction_sign,
                       bool is_put,
                       ...
                       QuantLib::ext::shared_ptr<QuantLib::YieldTermStructure> domestic_curve,
                       QuantLib::ext::shared_ptr<QuantLib::YieldTermStructure> foreign_curve)
        : notional_(notional), strike_rate_(strike_rate), direction_sign_(direction_sign), is_put_(is_put),
          expiry_date_(std::move(expiry_date)), settlement_date_(std::move(settlement_date)),
          spot_quote_(std::move(spot_quote)), volatility_quote_(std::move(volatility_quote)),
          domestic_curve_(std::move(domestic_curve)), foreign_curve_(std::move(foreign_curve)) {
        registerWith(spot_quote_);
...
protected:
    void performCalculations() const override {
        ...
        if (time_to_expiry <= 0.0 || strike_rate_ <= 0.0 || spot <= 0.0) {
            const double intrinsic = is_put_ ? std::max(strike_rate_ - spot, 0.0) : std::max(spot - strike_rate_, 0.0);
        ...
        const double unit_value =
            is_put_ ? strike_rate_ * domestic_df * normal_cdf(-d2) - spot * foreign_df * normal_cdf(-d1)
...
FxInstrumentFactory::create_fx_spot(const domain::FxSpotTrade& trade, const analytics::PricingContext& context) {
    const std::string quote_id =
        trade.spot_quote_id.empty() ? fx_pair(trade.base_currency, trade.quote_currency) : trade.spot_quote_id;
    auto spot_quote = context.market_state().get_quote_handle(quote_id);
    if (!spot_quote) {
        return nullptr;
    }

    return QuantLib::ext::make_shared<FxSpotInstrument>(trade.notional,
...
QuantLib::ext::shared_ptr<QuantLib::Instrument>
FxInstrumentFactory::create_fx_forward(const domain::FxForwardTrade& trade, const analytics::PricingContext& context) {
    const std::string quote_id =
        trade.spot_quote_id.empty() ? fx_pair(trade.base_currency, trade.quote_currency) : trade.spot_quote_id;
    auto spot_quote = context.market_state().get_quote_handle(quote_id);
    if (!spot_quote) {
    ...
    auto forward_quote =
        trade.forward_quote_id.empty() ? nullptr : context.market_state().get_quote_handle(trade.forward_quote_id);
    ...
    return QuantLib::ext::make_shared<FxForwardInstrument>(trade.notional,
                                                           trade.forward_rate,
                                                           fx_direction_sign(trade.direction),
...
QuantLib::ext::shared_ptr<QuantLib::Instrument>
FxInstrumentFactory::create_fx_swap(const domain::FxSwapTrade& trade, const analytics::PricingContext& context) {
    const std::string quote_id =
        trade.spot_quote_id.empty() ? fx_pair(trade.base_currency, trade.quote_currency) : trade.spot_quote_id;
    auto spot_quote = context.market_state().get_quote_handle(quote_id);
    if (!spot_quote) {
    ...
    return QuantLib::ext::make_shared<FxSwapInstrument>(trade.notional,
                                                        trade.near_rate,
                                                        trade.far_rate,
                                                        fx_direction_sign(trade.direction),
...
QuantLib::ext::shared_ptr<QuantLib::Instrument>
FxInstrumentFactory::create_ndf(const domain::NdfTrade& trade, const analytics::PricingContext& context) {
    const std::string quote_id =
        trade.spot_quote_id.empty() ? fx_pair(trade.base_currency, trade.quote_currency) : trade.spot_quote_id;
    auto spot_quote = context.market_state().get_quote_handle(quote_id);
    if (!spot_quote) {
    ...
    const auto base_currency = domain::from_string(trade.base_currency);
    auto forward_quote =
        !trade.fixing_quote_id.empty() ? context.market_state().get_quote_handle(trade.fixing_quote_id) : nullptr;
...
QuantLib::ext::shared_ptr<QuantLib::Instrument>
FxInstrumentFactory::create_fx_option(const domain::FxOptionTrade& trade, const analytics::PricingContext& context) {
    const std::string quote_id =
        trade.spot_quote_id.empty() ? fx_pair(trade.base_currency, trade.quote_currency) : trade.spot_quote_id;
    auto spot_quote = context.market_state().get_quote_handle(quote_id);
    if (!spot_quote) {
    ...
    if (!volatility_quote) {
        volatility_quote =
            QuantLib::ext::make_shared<QuantLib::SimpleQuote>(trade.volatility > 0.0 ? trade.volatility : 0.10);
    }
    ...
    const auto settlement_date =
        trade.settlement_date.empty() ? expiry_date : market::CurveBuilder::parse_date(trade.settlement_date);

    return QuantLib::ext::make_shared<FxOptionInstrument>(trade.notional,
                                                          trade.strike_rate,
                                                          fx_direction_sign(trade.direction),
