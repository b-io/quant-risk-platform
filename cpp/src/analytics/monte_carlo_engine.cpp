...
SimulationResult MonteCarloEngine::run_simulation(
    ...
    if (config.mode == MonteCarloMode::AgedHorizonRevaluation) {
        ...
        // Ensure non-curve quotes are available for product builders and shock resolution.
        for (const auto& q : base_market_dto.quotes) {
            if (!horizon_state->get_quote_handle(q.id)) {
                horizon_state->add_quote(q.id, q.value);
            }
    ...
    // Find lower triangular L such that Sigma = L * L^T.
    QuantLib::Matrix L = QuantLib::CholeskyDecomposition(repaired_cov);

    // Use a deterministic RNG seed for reproducible path generation.
    ...
    for (int i = 0; i < config.num_paths; ++i) {
        ...
        // Capture pre-shock quote levels for trace diagnostics.
        if (capture_trace) {
        ...
        if (capture_trace) {
            ...
            trace.path_pnl = total_npv_path - active_base_npv;
        ...
        // Aged horizon summaries report total P&L relative to t0.
        if (config.mode == MonteCarloMode::AgedHorizonRevaluation) {
    ...
    // Restore the active state before returning so callers can reuse it.
    active_state->reset_to_snapshot(base_market_dto);
    QuantLib::Settings::instance().evaluationDate() = t0;

    // Calculate risk metrics from sorted path P&L.
