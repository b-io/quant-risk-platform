...
void bind_market(py::module_& m) {
    ...
    m.def(
        "build_rates_market_report",
        [](const domain::MarketSnapshot& dto) { return market::RatesMarketBuilder::build(dto).curve_results; },
        "Build supported rates market objects and return curve build diagnostics");

    m.def("resolve_quote_values",
          &market::FactorShockResolver::resolve_quote_values,
          py::arg("scenario"),
          py::arg("factors"),
          py::arg("bindings"),
