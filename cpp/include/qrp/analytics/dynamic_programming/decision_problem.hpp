...
public:
     ...
     */
    virtual std::vector<Action> feasibleActions(const State& state, std::size_t timeIndex) const = 0;

    /**
     * @brief Returns the immediate cashflow from applying an action.
     */
    virtual double immediateCashflow(const State& state, const Action& action, std::size_t timeIndex) const = 0;

    /**
     * @brief Evolves operational state after an action and next market state.
     */
    virtual State nextState(const State& state,
                            const Action& action,
                            const std::vector<double>& market_variables_next,
                            std::size_t timeIndex) const = 0;

    /**
     * @brief Returns whether an action ends the decision process.
     */
    virtual bool isTerminalAction(const State&, const Action&, std::size_t) const {
        return false;
    }
