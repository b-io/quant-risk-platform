# Macro Regimes, Event Flows, GDP, and PMI
...
- $C$, $I$, $G$, $X$, and $M$ are consumption, investment, government spending, exports, and imports.
- $\pi$ is inflation and $\pi^*$ is the inflation target.
- $y$ is actual output and $y^*$ is potential output.
- $i$ is the policy rate.
- $r^*$ is the neutral real rate.
- $\text{output gap} = y-y^*$.
...
## 2. The main event types that trigger regimes
...
- fiscal stimulus
- credit expansion
- stronger household income / spending
- inventory rebuild
...
GDP is the broadest standard measure of economic activity.
...
### 3.2 Nominal GDP vs. real GDP

Nominal GDP includes price effects.
Real GDP strips out price changes.

Very roughly:

$$
\text{real GDP growth} \approx \text{nominal GDP growth} - \text{GDP deflator inflation}
$$
...
A standard diffusion-index formula is:

$$
PMI = (\%\text{higher}) + 0.5 \times (\%\text{unchanged})
$$
...
- often a good early signal for manufacturing and cyclical activity

It is not GDP, but it often helps nowcast GDP.
...
$$
\text{GDP} = \text{level / growth of broad output}
$$
...
$$
\text{PMI} = \text{breadth and direction of recent change}
$$

So PMI can turn down **before** GDP growth turns down, because firms see orders and activity weakening early.
...
## 5. How all the indicators link together
...
- **payrolls / unemployment / vacancies / wages** tell you how tight labor is.
- **CPI / core / PPI / breakevens** tell you whether inflation is temporary or persistent.
- **rates / spreads / FX / equities** reflect how markets think the central bank will react.

---

## 6. The main macro regimes
...
- vacancies cool faster than payrolls
- central bank pauses, then cuts gradually
...
- credit stress often appears
- inflation cools with a lag
- sovereign yields fall, credit spreads widen
...
- equities and cyclical risk recover
