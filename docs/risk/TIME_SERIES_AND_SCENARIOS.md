...
### Stationarity
A stationary process has stable statistical properties over time. Many raw financial series such as yields or prices are not stationary, but returns, changes, or spread changes may be closer to stationary.
...
is critical. Rates risk often uses **level changes in basis points**. Equity risk often uses **returns**. Credit risk often uses **spread changes** or relative spread shocks.
