...
The reusable LSMC module handles exercise-policy examples, Python-facing American option valuation, and product-specific
approximations separately. A general multi-step exposure cube and path-dependent pricing service remains an extension
area.
...
4. LSMC is exposed through a C++-managed helper, but the shared exercise-policy layer is not yet wired across every
