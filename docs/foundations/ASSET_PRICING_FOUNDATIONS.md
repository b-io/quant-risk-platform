...
- payoffs that arrive in adverse states of the world are more valuable,
...
For implementation details, see `docs/foundations/MARKOWITZ_PORTFOLIO_THEORY.md`.
...
These assumptions are too restrictive for literal use but the model remains foundational.

### 5.4 Practical Role Of CAPM
...
### 6.4 Role Of APT In Modern Risk Systems
...
- assets that pay off in adverse consumption states hedge the investor,
- those assets command lower expected returns,
- assets that perform poorly in adverse states require higher premia.
...
- macro and rates workflows naturally use these state variables,
...
| CCAPM / ICAPM            | Which state exposures are priced in equilibrium?            | Macro interpretation, research frameworks, long-horizon allocation |
...
| Structural credit        | How does firm-value risk map to default?                    | Economic credit interpretation, structural analysis           |
...
alternatives. They address different but connected questions.
