...
## 1. Role Of Characteristic Functions
...
## 3. Interpretation of the Fourier Transform
...
$$
\varphi_{X+Y}(t) = \mathbb{E}\left[e^{it(X+Y)}\right] = \mathbb{E}\left[e^{itX} e^{itY}\right].
$$

By independence,

$$
\mathbb{E}\left[e^{itX} e^{itY}\right] = \mathbb{E}\left[e^{itX}\right] \mathbb{E}\left[e^{itY}\right] = \varphi_X(t)\varphi_Y(t).
$$
