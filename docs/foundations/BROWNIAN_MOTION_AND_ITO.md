# Brownian Motion, Wiener Process, and Ito Calculus
...
The physical interpretation comes from the historical study of small particles suspended in a fluid whose irregular motion
...
$$
\mathrm{Var}\left(W_t^{(\Delta t)}\right) = \left\lfloor \frac{t}{\Delta t} \right\rfloor a_{\Delta t}^2 \approx \frac{t}{\Delta t} a_{\Delta t}^2.
$$
...
### 2.3 Scaling Of Brownian Increments
...
$$
\mathrm{Var}\left(W_t^{(\Delta t)}\right) \approx \frac{t}{\Delta t} (\Delta t)^{2\alpha} = t (\Delta t)^{2\alpha - 1}.
$$
...
### 2.4 Central Limit Theorem Interpretation
...
$$
W_t^{(\Delta t)} = \sqrt{n_{\Delta t} \Delta t}
...
$$
\frac{1}{\sqrt{n_{\Delta t}}} \sum_{k=1}^{n_{\Delta t}} X_k \Rightarrow \mathcal{N}(0,1).
$$
...
#### 2.4.1. Linear Time Scaling Of Variance
...
$$
\mathrm{Var}\left(\sum_{k=1}^{N} Y_k\right) = \sum_{k=1}^{N} \mathrm{Var}(Y_k) \approx N \cdot \Delta t \approx \frac{t}{\Delta t} \cdot \Delta t = t.
$$
...
Matching the marginal distribution at one time is insufficient. The **entire path-valued process** must converge.
The precise theorem is the functional central limit theorem, also called **Donsker's invariance principle**:

$$
W_t^{(\Delta t)} = \sum_{k=1}^{\lfloor t/\Delta t \rfloor} \sqrt{\Delta t} X_k \Longrightarrow W_t,
$$
...
$$
\mathrm{Var}\left(\sum_{k=1}^N Y_k\right) = \sum_{k=1}^N \mathrm{Var}(Y_k).
$$
...
$$
\mathrm{Var}\left(\sum_{k=1}^{t/\Delta t} \sqrt{\Delta t} X_k\right) = \frac{t}{\Delta t} \cdot \Delta t = t.
$$
...
- it is a natural example for Ito's formula
...
$$
S_1 = 100 \exp\left((0.05 - 0.5 \cdot 0.2^2) \cdot 1 + 0.2 \cdot 0.3\right) = 100 \exp(0.09) \approx 109.42.
$$

---

## 6. What is an Ito process?
...
This is useful for rates or spread-factor interpretation because the process tends to revert toward $\theta$.

---

## 7. Ito's formula: the stochastic chain rule
...
## 8. Step-by-step example of Ito's formula
...
A risk engine should not embed stochastic models directly into the pricing code. A clean architecture separates:
...
Starting from random walks, it leads naturally to Ito processes of the form

$$
dX_t = a(t,X_t)dt + b(t,X_t)dW_t.
$$

Ito's formula then tells us how functions of stochastic processes evolve, adding the crucial correction term
