...
This document is the canonical market-data and construction note for volatility in the platform. It is written as a
technical reference for surface representation, construction, validation, and risk mapping.
...
## 1. Definition of a Volatility Surface
...
## 2. Role of Implied Volatility

A vanilla option price can be inverted into the volatility that makes a benchmark model reproduce that price.

For equities, that benchmark is usually Black-Scholes-Merton. For forwards and futures, it is often Black 76. For rates
products, the quoting convention may be:

- normal volatility,
- lognormal volatility,
- shifted lognormal volatility.

Implied volatility does not assert that the benchmark model is structurally correct. It provides a stable quoting
language for option prices.

Volatility surfaces therefore serve three roles:
...
If the schema loses those distinctions, the platform may still run while producing inconsistent pricing and risk.
...
- market quotes use normal vols for swaptions in low-rate regimes.

Trade-off:

- negative rates are allowed naturally, but large positive rates may be less plausible in some contexts.
...
- the shift is part of the market convention.
...
- more aligned with variance accumulation,
...
- broader validation and risk review scope.
...
- can encode market-shape assumptions.
...
Volatility surfaces are constrained market objects. Poor interpolation can imply static arbitrage.
...
Many platforms adopt a layered approach:
...
The implementation objective is a surface that is stable, explainable, close to liquid market quotes, and resistant to
numerical fragility.
...
$$
\sigma(0.75)=
\sqrt{\frac{0.0388}{0.75}} \approx 22.75\%.
$$

This interpolation is often more stable than directly interpolating $20\%$ and $24\%$ linearly in volatility space.
...
Therefore `vol_type` must be part of the surface identity, not only an attribute for display.
...
The implementation implication is that the surface object may be strike-based internally even when the market is
delta-based externally.
...
- sticky delta may better reflect FX market quoting behavior,
- model-consistent re-marking is theoretically cleaner but operationally heavier and less transparent.

Modern platforms often support more than one rule because pricing, market risk, and model validation may each require
different scenario semantics.

---

## 11. Platform Representation
...
- **exercise-style and option-pricing choices** - see `docs/models/volatility/OPTION_PRICING_AND_EXERCISE_STYLES.md`,
...
- map vega to stable risk buckets,
- and keep scenario semantics explicit rather than hidden.

The surface is both a market object and a risk object. It should therefore be engineered with the same care as discount
