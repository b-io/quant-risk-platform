...
There are two distinct tasks that are often conflated:

1. **Static market-curve construction today**
2. **Dynamic modeling of how rates, spreads, or prices evolve through time**

For today's pricing and risk, production platforms usually start from **bootstrapped market curves**.
...
As a standalone factor process, it is useful for interpretation and simulation, but it is not by itself a full
...
Rates modeling often separates **arbitrage-free models** from **equilibrium models**.
...
- for **pricing today**, the platform usually needs exact or near-exact market fit;
- for **economic interpretation or long-run dynamics**, an equilibrium model may still be informative.

---

## 2. Short-rate models for interest rates

### 2.1 Distinct Modeling Requirements For Rates
...
#### Example for 2.2 - Ho-Lee is called arbitrage-free in practice
...
valuation because it does not guarantee an exact fit to today's observed term structure.
...
#### Example for 2.3 - Vasicek is called an equilibrium model
...
A common equivalent interpretation is
...
- mean reversion produces more plausible rate dynamics than Ho-Lee,
...
The two models address related questions with different calibration properties.

- **Ho-Lee** is the cleanest no-arbitrage normal short-rate model and is useful for explaining curve-fitting mechanics
...
#### Example for 2.5 - Mean reversion matters
...
That makes Vasicek useful for equilibrium interpretation and Hull-White more suitable for market-consistent medium-horizon
rate scenarios.
...
For today's plain-vanilla discount curve itself, production systems usually start from **market bootstrap**, not from a
short-rate model alone.

#### Example for 2.6 - static and dynamic rates layers

For a USD collateralized rates workflow:
...
For vanilla rates options, models often use a forward rate or swap rate directly rather than only the short rate.
...
- the **normal** model allows negative rates naturally and is widely used when market quotes are normal vols,
...
$$
F_{0.25} \approx 2.50\% \times e^{-0.5\cdot 0.012^2\cdot 0.25 + 0.012\cdot 0.5\cdot 0.5} \approx 2.5075\%
$$
...
For today's CDS pricing, production systems usually use:
...
They are less often the primary production representation for high-precision OIS discounting, where exact repricing of
liquid instruments is usually more important than a very low-dimensional parametric fit.

#### Example for 4.3 - static trading curve versus macro scenario curve

A USD OIS discount curve may need to reprice many liquid SOFR OIS quotes almost exactly.
...
For today's OIS discount curve in USD, EUR, or GBP, the standard production choice is usually:
...
A valuation workflow may build:
...
#### Example for 5.2 - central-bank publication versus instrument-exact curve

A central bank may publish a smooth Nelson-Siegel-Svensson sovereign zero curve.
A valuation system may still use a more instrument-exact bootstrapped curve for P&L and hedging.
...
A market source may quote a 5Yx10Y swaption in normal vol and fit the smile with SABR.
...
- **Ho-Lee**: exact initial-curve fit and simple mechanics, but no mean reversion.
...
The resulting implementation taxonomy is: **CIR is low-dimensional and affine**, **Hull-White is curve-consistent and
tractable**, and **LMM is richer but computationally heavier**.

#### Example for 5.4 - calibration target drives model choice

If the requirement is a callable swap benchmark and fast Greeks, one-factor Hull-White may be sufficient. If the
requirement is a large caplet or swaption surface with forward-rate structure, LMM is more natural.
...
- GBM is still the simplest reference model for interpretation and Monte Carlo,
...
For an option portfolio with smile and skew risk, the model stack will usually need local vol, stochastic vol, or a calibrated
implied-vol surface.
...
The modeling hierarchy can be summarized as:
...
That is the main implementation distinction: current market-state construction is a bootstrapping problem, while future
factor evolution is a modeling problem.
