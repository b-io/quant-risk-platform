...
### 1.2 Discrete To Continuous Compounding
...
#### Example for 1A - bond PV calculation
...
- central for rates and cross-asset pricing.
...
- US Treasuries,
- German Bunds,
- UK gilts, and other sovereign benchmarks.
...
$$
\text{corporate yield} \approx
\text{base curve yield} + \text{credit spread} + \text{liquidity spread}
$$

where the base curve may be sovereign or OIS depending on the valuation context.
...
## 4. Yield Behavior In Recessions
...
### 4.3 Curve Inversion And Recession Interpretation
...
The following worked example uses a simplified USD SOFR OIS discount curve.

### 5.1 Raw Market Data Representation

Vendor mnemonics should not be hard-coded in analytics code.
...
A representative normalized quote set is:
...
Some curve sources also use short-rate futures to anchor the front end.
...
$$
P(0,1Y) = \frac{1}{1 + K_1} = \frac{1}{1 + 0.0270} = 0.973710
$$
...
## 6. Bootstrap Technique Selection
...
- interpretable,
- auditable,
- stable under normal valuation operations.

#### Example for 6.1 - Sequential matters operationally
...
## 7. How to implement this concretely in a pricing system

A clean implementation flow is:

$$
\text{raw quotes} \rightarrow
\text{normalized market records} \rightarrow
\text{convention resolution} \rightarrow
\text{curve helpers} \rightarrow
\text{piecewise bootstrap} \rightarrow
\text{market state} \rightarrow
...
must be separated in a real analytics platform.
