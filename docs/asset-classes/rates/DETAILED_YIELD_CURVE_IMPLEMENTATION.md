# Detailed Yield-Curve Implementation in Practice (USD / EUR / GBP)
...
- how a production curve is built,
- what raw quotes are needed,
- what instrument sets are typical in major OIS markets such as USD, EUR, and GBP,
...
- which trade-offs are typically preferred in a pricing and risk platform.
...
## 2. Production Curve Stack

In a modern rates platform, "the yield curve" is a curve set rather than a single object.
...
$$
\texttt{discount(Date)} \rightarrow
t \rightarrow
\texttt{discountImpl}(t) \rightarrow
P(0,t)
$$
...
### 4.2 `discountImpl` As The Curve Primitive
...
- `forwardRate(...)` for rate reporting,
...
> Vendor tickers should not be hard-coded directly into pricing logic.
...
| `GBP-RFR-ON`  | GBP overnight risk-free fixing |
| `GBP-OIS-10Y` | 10-year GBP OIS par swap       |
...
- GBP SONIA overnight reference: vendor identifiers depend on source and entitlement
...
- ICE three-month SONIA futures, where used by the curve source
...
> should stay in the market-data adapter because vendor symbology is not stable enough for pricing code.
...
For macro or credit-spread analysis, systems often track sovereign curves such as:
...
The standard collateralized discount curve is the **SONIA OIS curve**.

Typical raw inputs:

- SONIA overnight reference and fixings
- short SONIA OIS or money-market style inputs
- SONIA futures if used by the curve source
- SONIA OIS swaps out the curve
...
- UK government bonds provide the sovereign benchmark term structure.

This can diverge from SONIA OIS because:

- sovereign cash bonds embed supply/demand and repo effects,
- SONIA is rooted in the sterling overnight funding market.

---

## 7. How to build an OIS discount curve concretely

The production method is not an unconstrained fit through quoted yields. The production method is:
...
- overnight to ~3M: overnight fixings, short OIS, or cash-style instruments if consistent with the curve convention,
...
#### Example for 7.6 - validation failure
...
$$
\text{macro data} \rightarrow
\text{policy expectation} \rightarrow
\text{front-end repricing} \rightarrow
...
There is only the best choice **for a given valuation objective**.
...
- OIS discount curves,
- general pricing and risk engines,
...
- not the first choice for the main discount curve.
...
- susceptible to overfitting noisy quotes,
...
> Splines are visually smooth, but for a discount curve robustness, monotonicity, and explainability usually
...
- not as the sole discounting curve.

---

## 10. What extrapolation policy is best

No universal interpolation policy exists.
...
- often overkill for macro portfolios.
...
One concrete recommendation for a macro, rates, or credit platform is:
...
### Limitations Of Direct Yield Interpolation
...
### Limitations Of Nelson-Siegel For Live Pricing

Because it is a factor fit, not an exact tradable-instrument bootstrap, so it is better for scenario design and macro
analysis than for primary pricing.

### Separation Of OIS And Sovereign Curves
...
> In practice, USD, EUR, and GBP OIS discount curves should be built from normalized overnight and swap-market inputs,
...
| Cross-currency basis or more specialized helpers | specialized / extended helper sets |
...
auto sonia = ext::make_shared<Sonia>(discountHandle);
```
