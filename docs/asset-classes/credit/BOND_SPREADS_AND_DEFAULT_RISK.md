...
$$
This approximation is compact but can be misleading when coupon structures differ materially from the benchmark.
...
For a plain cash bond, market commentary may refer informally to the bond spread as the difference between the bond yield
and a benchmark government or swap yield. This is useful for preliminary market comparison, but it is not stable across coupons or
cashflow profiles.
...
measure. It is widely used because it is simple to compute and compare across bonds.
...
market/par, and the floating index used. The key interpretation is:
...
Because the two measures address different questions, they can differ materially.
...
$$
This is not a production pricing formula, but it is useful as a first-order approximation. It shows that a higher
...
### 7.2 Clean Labeling Requirements
...
- `docs/asset-classes/credit/CREDIT_CURVE_CONSTRUCTION.md` explains how reusable credit curves are built and wired into the
  platform.
- `docs/asset-classes/credit/CDS_CURVES_AND_CREDIT_RISK_IN_PRACTICE.md` explains CDS cashflows, calibration logic, and credit
  usage patterns.
- `docs/risk/RISK_MEASURES_AND_EXPLAIN.md` and `docs/risk/PNL_EXPLAIN_IN_PRACTICE.md` explain how these quantities feed
  into risk, explain, and scenario reporting.
